...
use sokol::{app::{self as sapp, Event}, gfx as sg, glue as sglue};
...
extern "C" fn event_cb(event: *const Event){
    let event = unsafe { &mut *(event as *mut Event) };

    println!("Some event was triggered {} {:}", event.char_code, event._type);
}
